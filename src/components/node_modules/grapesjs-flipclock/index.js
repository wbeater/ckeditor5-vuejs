...
function loadBlocks(editor, opt = {}) {
    ...
    if (c.blocks.indexOf(countdownRef) >= 0) {
      var config = JSON.stringify(c['data-config'] || {});
      bm.add(countdownRef, {
        ...
        content: `
          ...
          <div class="${pfx} flip-clock-wrapper" data-config='${config}'></div>
    ...
    domc.addType(COUNTDOWN_TYPE, {
        ...
        isComponent(el) {
          if(
            el && el.classList && el.classList.contains('flip-clock-wrapper')
              ...
              if (config) {
                return {...JSON.parse(config), type: COUNTDOWN_TYPE};
              }
      ...
      view: defaultView.extend({
        init() {
          ...
          this.listenTo(this.model, 'change', (model, value) => {
            if (model && model.attributes && model.attributes.attributes) {
              const {language,startfrom,clockFace,countDown,showSeconds,endText} = model.attributes;
              model.attributes.attributes['data-config'] =  JSON.stringify({language,startfrom,clockFace,countDown,showSeconds,endText});
            }
