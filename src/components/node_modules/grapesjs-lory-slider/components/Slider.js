    ...
    model: defaultModel.extend({
      defaults: {
        ...
        script() {
          ...
          //BEGIN - Infinite
          var startTime, fpsInterval, slideSpeed, now, then, elapsed, running=false;
          
          var startAnimating = function() {
            let fps = options.slideSpeed / 1000;
            fpsInterval = 1000 / fps;
            then = Date.now();
            startTime = then;
            if (window.sliderLory) animate();
          }
          ...
          var animate = function() {
            ...
            if (elapsed > fpsInterval) {
              ...
              if (options.infinite) window.sliderLory.next();
            }
          }
          //END - Infinite
          ...
          var initSlider = function() {
            ...
            if (!options.infinite) window.started = false;
            if (options.infinite && options.slideSpeed) {
              if (!window.started) {
                startAnimating();
                window.started = true;
              }
      ...
      init() {
        const cls = config.classSlider;
        this.get('classes').pluck('name').indexOf(cls) < 0 && this.addClass(cls);
      }
    }, {
      isComponent(el) {
        if (elHasClass(el, config.classSlider)) return { type: sliderName };
